...
/* ============================================================
 * USART1: PA9=TX, PA10=RX, 2 Mbps 8N1 (板载 CH340G; 2026-09-16 从 115200 提升,
 *         波特率定义在 uart.c 的 BOARD_BAUD —— 改那里, 别改这里)
 * 指令协议 (单字符, 大小写不敏感, 行结束符忽略, 测量期间忽略):
 *   S = 单次测量 (1s 窗口; <1nA 自动换 10s 窗口重测一次, 测完自动回空闲)
 *   N = 底噪标定 (50s 纯积分)
 *   D = 查询最近结果
 *   B = 回传最近完整窗口原始波形 (WAVE 头 + 二进制数据 + 校验和)
 *   X = 回传同一窗口的 ADS8866 外部 16 位波形 (WAVEX 头, 格式同 B)
 *   E = ADS8866 观测通路诊断一行 (样本数/坏读数/SPI 超时计数)
 *   M = 手动阻塞连采 (整行, 需带子模式): M0=断开参考(量 tau),
 *       M1=固定+5V / M2=固定-5V (q 的开环标定)。完成后用 B / X 回传 6250 点
