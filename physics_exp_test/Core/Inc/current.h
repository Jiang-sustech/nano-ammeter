 ...
 * 设 0 可以关掉, 用来做对照实验。
 *
 * ⚠️ **这个宏从引入起就不是 0** (2026-09-15 查 git 确认): 它和 `DelayUs()`、
 * `USE_INTERNAL_ADC 0` 是**同一个提交 be3a750 (2026-09-13 23:01) 一次性引入**的,
 * 当时是 20U, 后来定档 8U。
 * 后果: `DelayUs()` 裸读 DWT->CYCCNT 而启用 DWT 的代码在 ads8866.c 里、调用
 * 顺序上排在它**后面** -> CYCCNT 恒为 0 -> **`S` 一按就死循环在 TIM6 中断里**。
 * **也就是说从 be3a750 起, 这个固件一直跑不了 `S`**, 直到 2026-09-15 在 main()
 * 里显式启用 DWT 才修好 (commit e0d9bcd)。
 * 之所以两天没人发现: 那之后**从未在真机上跑过 `S`** —— 全部实测数据都早于
 * be3a750 (最晚的 npz 是 09-13 17:46), 标定数据是更早的固件 + 手动设源表采的。 */
