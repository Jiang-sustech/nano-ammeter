...
def smu_open(res):
    ...
    #
    # 取 5V 的依据: 模拟前级轨是 ±5V (ADG1219 在 ±5V/100MΩ 间切), 导通阈值 5.3V。
    #
    # ⚠️ 待验证两点:
    #   ① 运放电源轨**请按实物确认**。若实际是 ±12V, 这里可以放宽到 12V。
    #   ② 正常工作时所需电压必须**远低于** 5V, 否则 20nA 以上会静默截顶、
    #      读数偏小而看不出来。用户观察 2 nA 时只有 0.5V, 但那 0.5V 是线性的
