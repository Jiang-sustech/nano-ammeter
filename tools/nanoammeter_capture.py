# -*- coding: utf-8 -*-
"""
firmware-v0: full UART data return + plot
...
--no-noise: 跳过 N/W 两步。firmware-v1 已删除底噪指令 (见该工程
...
# 两代结果行的并集。firmware-v0:  "I=+25.274 nA MODE=1"
#                      firmware-v1: "I=<内置> X=<外部> MODE=1 T=1000ms [CAL=..]
...
# firmware-v1 的 MODE=1 会出。存进 npz 是为了让 (m+n) 与 (m+n-1) 两种
...
def main():
    ...
    if MANUAL is not None:
        ...
        ser.write(("M%d\n" % sub).encode())
        # ⚠️ 等待上限必须覆盖固件最长窗口。firmware-v1 的自动路径在
