...
// 初始化序列按官方说明书 (HS13L01 规格书 4.1 参考代码 OLED_Init)
void SH1106_Init(void) {
	// Hardware display reset
	// 本模块 (HS13L01) 无 RES 引出脚, 靠内部上电复位, RST 宏为空操作
	SH1106_CS_H();
	SH1106_RST_L();
	SH1106_RST_H();
	SH1106_CS_L();
	...
	SH1106_cmd(SH1106_CMD_DISP_OFF); // 0xAE: display off
	SH1106_cmd(SH1106_CMD_COL_LOW | 0x02); // 0x02: lower column address (SH1106 132列, 显示起始列偏移2)
	SH1106_cmd(SH1106_CMD_COL_HIGH); // 0x10: higher column address
	SH1106_cmd(SH1106_CMD_STARTLINE | 0x00); // 0x40: set display start line
	SH1106_cmd(SH1106_CMD_PAGE_ADDR | 0x00); // 0xB0: set page address
	SH1106_cmd_double(SH1106_CMD_CONTRAST, 0xFF); // 0x81,0xFF: contrast control (用户要求亮度最大, 官方示例 0xCF)
	SH1106_cmd(SH1106_CMD_SEG_INV); // 0xA1: set segment remap
	SH1106_cmd(SH1106_CMD_INV_OFF); // 0xA6: normal display
	SH1106_cmd_double(SH1106_CMD_SETMUX, 0x3F); // 0xA8,0x3F: multiplex ratio, duty = 1/64
	SH1106_cmd_double(0xAD, 0x8B); // charge pump enable, 0x8B: 内供 VCC
	SH1106_cmd(0x33); // 0x30~0x33: set VPP 9V
	SH1106_cmd(SH1106_CMD_COM_INV); // 0xC8: COM scan direction (remapped)
	SH1106_cmd_double(SH1106_CMD_SETOFFS, 0x00); // 0xD3,0x00: set display offset
	SH1106_cmd_double(SH1106_CMD_CLOCKDIV, 0x80); // 0xD5,0x80: set osc division
	SH1106_cmd_double(0xD9, 0x1F); // pre-charge period
	SH1106_cmd_double(SH1106_CMD_COM_HW, 0x12); // 0xDA,0x12: set COM pins
	SH1106_cmd_double(0xDB, 0x40); // VCOMH 0.77 x VCC (官方参考代码 0xDB,0x40)

	// Display ON
	SH1106_cmd(SH1106_CMD_DISP_ON); // 0xAF: display enabled
