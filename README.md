...
**⚠️ 三个常数的精度差约 70 倍，脚本会把这件事直接打出来。**

| 项 | 跨度 | 结果（N=200、码噪声 68 码） |
|---|---|---|
| 计数项 `(I₊−I₋)·a₂` | **60 nA** | `I₊`/`I₋` 定到 **0.0004%** |
| 端点项 `M·a₁` | **846 pA** | `M`（即 `q`）定到 **0.032%** |

**两个都够用** —— 摊到单次读数上分别只贡献 0.02 pA 和 0.26 pA，都远低于仪器
1 pA 的分辨力。

**物理原因**：电荷平衡环把信息放进了**占空比**，把端点差压成了**残差**。
所以若想把 `q` 再压低一个量级，得**把环路打开**（固定参考极性、灌已知电流、
让 `Δc` 跑满整条斜坡，重复 N 次）—— 算法不用换，**换的是实验条件**。

> 完整推导（含一个"合成数据量级取错导致结论反过来"的教训）见
