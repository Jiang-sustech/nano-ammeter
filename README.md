...
**⚠️ 合成数据实测到一个关键限制：三个参数的"杠杆"差 960 倍。**

| 项 | 跨度 | 结果 |
|---|---|---|
| 计数项 `(I₊−I₋)·a₂` | **60 nA** | `I₊`/`I₋` 定到 **0.0004%** |
| 端点项 `M·a₁` | **62 pA** | `M` 只能定到 **~0.45%** |

**所以分工应该是：**

- **`I₊`/`I₋` 从这个拟合拿**（极准，而且能和 `V/R` 独立对照）
- **`q` 用专门的"灌已知电流"测量拿** —— 那里 `Δc` 能跑满整条斜坡，单点杠杆比这里
  大一个量级，重复 N 次还能再降 `√N`

脚本会给每个参数**标准误**，并自动判断杠杆差距、在数据退化时报警（条件数 → 1e17）。
